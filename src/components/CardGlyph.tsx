...
export default function CardGlyph({ variant }: { variant: GlyphVariant }) {
  return (
    ...
    >
      {variant === "chip" && (
        <>
          <rect x="44" y="24" width="52" height="52" rx="6" stroke={S} strokeWidth="1.3" />
      ...
      {variant === "nodes" && (
        <>
          ...
          <path d="M76 33 L54 52 M54 64 L84 74 M88 37 L91 69" stroke={S} strokeWidth="1.1" />
      ...
      {variant === "tune" && (
        <>
          <path d="M32 36 H100 M32 60 H100 M32 84 H100" stroke={S} strokeWidth="1.2" />
          <circle cx="76" cy="36" r="6" fill="#f4f3ef" stroke={S} strokeWidth="1.3" />
          <circle cx="52" cy="60" r="6" fill="#f4f3ef" stroke={S} strokeWidth="1.3" />
          <circle cx="88" cy="84" r="6" fill="#f4f3ef" stroke={S} strokeWidth="1.3" />
        </>
      )}
      {variant === "pulse" && (
        <>
          <path d="M24 60 H46 L56 36 L68 84 L78 52 L84 60 H104" stroke={S} strokeWidth="1.4" strokeLinejoin="round" strokeLinecap="round" />
      ...
      {variant === "doc" && (
        <>
          ...
          <path d="M54 38 H84 M54 50 H78 M54 62 H84 M54 74 H70" stroke={S} strokeWidth="1.1" />
        </>
      )}
      {variant === "wave" && (
        <>
          <path d="M24 64 C 38 44, 50 44, 62 64 S 88 84, 102 64" stroke={S} strokeWidth="1.4" strokeLinecap="round" />
          <path d="M24 44 C 38 24, 50 24, 62 44 S 88 64, 102 44" stroke={S} strokeWidth="1.1" strokeLinecap="round" opacity="0.6" />
        </>
      )}
      {variant === "memory" && (
        <>
          ...
          <path d="M44 42 H88 M44 52 H72" stroke={S} strokeWidth="1.1" />
        </>
      )}
    </svg>
  );
